...
class FeedForward(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias):
        ...
        self.kernel = nn.Sequential(
            nn.Linear(512, dim*2, bias=False),
        )
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.kernel = nn.Sequential(
            nn.Linear(512, dim*2, bias=False),   #256->512
        )
    ...
    def forward(self, x,k_v):
        b,c,h,w = x.shape   #[3, 52, 256, 256]
        k_v=self.kernel(k_v).view(-1,c*2,1,1)  #[3, 256]->[3, 104, 1, 1]
        k_v1,k_v2=k_v.chunk(2, dim=1)
        x = x*k_v1+k_v2  
...
class DIRformer(nn.Module):
    def __init__(self, 
        ...
        self.latent = nn.Sequential(*[TransformerBlock(dim=int(dim*2**4), num_heads=heads[4], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[4])])

        self.up5_4 = Upsample(int(dim*2**4)) ## From Level 5 to Level 4
    ...
    def forward(self, inp_img,k_v):
        ...
        out_enc_level4,_ = self.encoder_level4([inp_enc_level4,k_v]) 

        inp_enc_level5 = self.down4_5(out_enc_level4)     
        latent,_ = self.latent([inp_enc_level5,k_v]) 
        ...
        out_dec_level4,_ = self.decoder_level4([inp_dec_level4,k_v]) 
...
class CPEN(nn.Module):
    def __init__(self,n_feats = 64, n_encoder_res = 6):     #n_feats=64
    ...
    def forward(self, x,gt):
        gt0 = self.pixel_unshuffle(gt)  #[3, 3, 256, 256]->[3, 48, 64, 64]
        x0 = self.pixel_unshuffle(x)  #[3, 4, 256, 256]->[3, 64, 64, 64]
        x = torch.cat([x0, gt0], dim=1)   #[3, 48, 64, 64]+[3, 64, 64, 64] -> [3, 112, 64, 64]
        fea = self.E(x).squeeze(-1).squeeze(-1)  #[3, 112, 64, 64]-> [3, 256]
        fea_S1 = []
...
class DiffIRS1(nn.Module):
    def __init__(self, 
        ...
        self.E = CPEN(n_feats=128, n_encoder_res=n_encoder_res)     #n_feat=64
