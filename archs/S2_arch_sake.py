...
class DIRformer(nn.Module):
    ...
    def forward(self, inp_img,k_v):
        ...
        #  #-------------------------------------------------------加入传统算法---------------------------------------------
        # print("type(out_dec_level1)=",type(out_dec_level1),"  shape=",out_dec_level1.shape)
        ...
        # #ww = io.loadmat('/home/b109/code/xx/DiffIR-master/DiffIR-master/DiffIR-inpainting/data/mri4000/weight/weight1_GEBrain.mat')['weight']
        ...
        # #mask_item=io.loadmat('/home/b109/code/xx/DiffIR-master/DiffIR-master/DiffIR-inpainting/data/mask/cart/mask_cart_030.mat')['mask']
        ...
        # #读取一个多线圈数据，制作sake使用的ksample
        # ori_input = np.zeros([256, 256, coil], dtype=np.complex64)
        # #ori_input =scipy.io.loadmat("/home/b109/code/xx/DiffIR-master/DiffIR-master/DiffIR-inpainting/data/mri4000/val_256/random_thick_256_test/1ori.mat")['Img2']
        ...
        # Kdata_sake_used = fft2c_noshift(ori_input) # max: 14.9250
        ...
        # #读取结果
        # trans_res_k_w = np.zeros((coil,256,256),dtype=np.complex64)
        # kw_real = np.zeros((coil,256,256),dtype=np.float32)
        # kw_imag = np.zeros((coil,256,256),dtype=np.float32) 
        # for i in range(coil):
        #     trans_res = out_dec_level1[i].permute(1, 2, 0).detach().cpu().numpy()
        #     kw_real[i,:,:] = trans_res[:,:,0]
        #     kw_imag[i,:,:] = trans_res[:,:,1]
        #     trans_res_k_w[i,:,:] = kw_real[i,:,:]+1j*kw_imag[i,:,:]
        ...
        # #除去权重+DC+转换为图像
        # for i in range(coil):
        #     k_complex[i,:,:] = wgt2k(trans_res_k_w[i,:,:],weight[i,:,:],Ksample[i,:,:])
        ...
        #   #K_end[:,:,i] = np.sqrt(fctr)*k_complex3[:,:,i]
        ...
        # x_output=np.random.uniform(-1, 1, size=(coil, 3, 256, 256))
        # for i in range(coil):
        #     k_w[i,:,:] = k2wgt(K_end[:,:,i],weight[i,:,:])  
        #     x_output[i,0,:,:] =  np.real(k_w[i,:,:])
        #     x_output[i,1,:,:] =  np.imag(k_w[i,:,:])
        #     x_output[i,2,:,:] =  np.real(k_w[i,:,:])
        # x_output = torch.from_numpy(x_output)
        # print("type(x_output)=",type(x_output),"    shape=",x_output.shape)


        # out_dec_level1=x_output
        # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        # out_dec_level1 = out_dec_level1.to(device)
