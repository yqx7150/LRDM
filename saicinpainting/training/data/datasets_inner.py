...
class InpaintingTrainDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # print("siat_kdata.shape=",siat_kdata.shape)
        # sys.exit()

        #将原数据中间抠掉,让模型只学高频数据，这里不需要乘weight
        width=25
        ...
        #siat_kdata_img = siat_kdata_img/np.max(np.abs(siat_kdata_img))
        # write_images(abs(mask),osp.join('/home/b109/code/xx/DiffIR-master/DiffIR-master/DiffIR-inpainting/data/temp/','siat_kdata_img'+'.png'))
        # data = {"kdata": siat_kdata}
        # savemat("/home/b109/code/xx/DiffIR-master/DiffIR-master/DiffIR-inpainting/data/temp/siat_kdata"+number+"_noLow.mat" , data)
        


        #加载权重
        weight = loadmat('/home/b109/code/xx/DiffIR-master/DiffIR-master/DiffIR-inpainting/data/mri4000/weight/weight1_GEBrain.mat')['weight']
        ...
        for x in range(128-width,128+width-1):
            for y in range(128-width,128+width-1):
                mask[0,x,y]=1
        mask = 1-mask
        # mask_img = np.sqrt(np.sum(np.square(np.abs(mask[0, :, :])),axis=0))
        # mask_img = mask_img/np.max(np.abs(mask_img))
        # write_images(abs(mask[0, :, :]),osp.join('/home/b109/code/xx/DiffIR-master/DiffIR-master/DiffIR-inpainting/data/temp/','mask'+'.png'))
