...
class InpaintingDataset(Dataset):
    def __init__(self, datadir, img_suffix='.jpg', pad_out_to_modulo=None, scale_factor=None):
        ...
        self.img_filenames = [fname.rsplit('_crop000_mask', 1)[0] + ".mat" for fname in self.mask_filenames]
    ...
    def __getitem__(self, i):
        ...
        #读取.mat文件
        siat_input =scipy.io.loadmat(self.img_filenames[i])
        siat_input = siat_input['k_w']   #256x256x3
        siat_input = siat_input.transpose(2, 0, 1)
        #print("siat_input.shape = ",siat_input.shape)
        mask = load_image(self.mask_filenames[i], mode='L')
        result = dict(image=siat_input, mask=mask[None, ...])
        ...
        #print("type(image)=",type(result['image']),"   shape =",result['image'].shape)
