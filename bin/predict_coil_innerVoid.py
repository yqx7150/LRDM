...
def main(predict_config: OmegaConf):
    try:
        ...
        #读取mask
        #======== mask
        mask_ori = np.zeros((coil,256,256))
        ...
        #零填充
        Kdata_ori = np.zeros((coil, 256, 256), dtype=np.complex64)
        Kdata = np.zeros((coil, 256, 256), dtype=np.complex64)
        Ksample = np.zeros((coil,256,256),dtype=np.complex64)
        Ksample_ori = np.zeros((coil,256,256),dtype=np.complex64)
        ...
        for i in range(coil):
            ...
            Ksample_ori[i,:,:] = np.multiply(mask_ori[i,:,:],Kdata_ori[i,:,:])  
        ...
        for i in range(coil):
            ...
            for x in range(128-width,128+width-1):
                for y in range(128-width,128+width-1):
                    k_complex2[i,x,y]=Ksample_ori[i,x,y]
