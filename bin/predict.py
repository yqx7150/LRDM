...
def main(predict_config: OmegaConf):
    try:
        ...
        for img_i in tqdm.trange(len(dataset)):
            ...
            #siat_input =scipy.io.loadmat("/home/b109/code/xx/DiffIR-master/DiffIR-master/DiffIR-inpainting/data/mri4000/train_256/17.mat")
            siat_input =scipy.io.loadmat("/home/b109/code/xx/DiffIR-master/DiffIR-master/DiffIR-inpainting/data/mri4000/val_256/val_source_256_test/"+number+".mat")
            # assert 0
            ...
            #加载权重
            weight = np.zeros((coil,256,256))  
            ww = loadmat('/home/b109/code/xx/DiffIR-master/DiffIR-master/DiffIR-inpainting/data/mri4000/weight/weight1_GEBrain.mat')['weight']
            for i in range(coil):
                weight[i,:,:] = ww
            ...
            for i in range(coil):
                ...
                kdata_w[i,:,:] = k2wgt(Kdata[i,:,:], weight)
                Ksample[i,:,:] = np.multiply(mask[i,:,:],Kdata[i,:,:]) # max: 3820.8044
                zeorfilled_data[i,:,:] = np.fft.ifft2(Ksample[i,:,:])  
            
            io.savemat(osp.join(save_path,img_name+'ori_Kdata.mat'),{'ori_Kdata':Kdata})

            Kdata_w = np.sqrt(np.sum(np.square(np.abs(kdata_w)),axis=0))
            Kdata_w = Kdata_w/np.max(np.abs(Kdata_w))
            write_images(abs(Kdata_w),osp.join(save_path,img_name+'k_w'+'.png'))
            ...
            # k_complex2[i,:,:] = np.fft.fftshift(k_complex2[i,:,:])
            io.savemat(osp.join(save_path,img_name+'rec_mask.mat'),{'mask':mask})
            ...
            io.savemat(osp.join(save_path,img_name+'Rec_kw.mat'),{'k_w':k_w})
            ...
            io.savemat(osp.join(save_path,img_name+'Rec_k_complex_noDc.mat'),{'Rec_k_complex_noDc':k_complex})
            rec_k_complex_sos = np.sqrt(np.sum(np.square(np.abs(k_complex)),axis=0))
            rec_k_complex_sos = rec_k_complex_sos/np.max(np.abs(rec_k_complex_sos))
            write_images(abs(rec_k_complex_sos),osp.join(save_path,img_name+'Rec_k_complex_sos'+'.png'))
            ...
            rec_Image_noDC_sos = np.sqrt(np.sum(np.square(np.abs(NCSNpp_Image_noDC)),axis=0))
            rec_Image_noDC_sos = rec_Image_noDC_sos/np.max(np.abs(rec_Image_noDC_sos))


            # Print PSNR
            psnr = compare_psnr(255*abs(rec_Image_noDC_sos),255*abs(ori_data_sos),data_range=255)
            ssim = compare_ssim(abs(rec_Image_noDC_sos),abs(ori_data_sos),data_range=1)
